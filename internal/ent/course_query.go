...
func (cq *CourseQuery) QueryMedia() *MediaQuery {
...
func (cq *CourseQuery) QueryCreator() *UserQuery {
...
// QueryCourseSections chains the current query on the "course_sections" edge.
func (cq *CourseQuery) QueryCourseSections() *CourseSectionQuery {
	query := (&CourseSectionClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CourseQuery) QueryCourseVideos() *VideoQuery {
...
func (cq *CourseQuery) QueryTests() *TestQuery {
	query := (&TestClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CourseQuery) WithMedia(opts ...func(*MediaQuery)) *CourseQuery {
	query := (&MediaClient{config: cq.config}).Query()
...
func (cq *CourseQuery) WithCreator(opts ...func(*UserQuery)) *CourseQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithCourseSections tells the query-builder to eager-load the nodes that are connected to
// the "course_sections" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CourseQuery) WithCourseSections(opts ...func(*CourseSectionQuery)) *CourseQuery {
	query := (&CourseSectionClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCourseSections = query
	return cq
}
...
func (cq *CourseQuery) WithCourseVideos(opts ...func(*VideoQuery)) *CourseQuery {
	query := (&VideoClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CourseQuery) WithTests(opts ...func(*TestQuery)) *CourseQuery {
	query := (&TestClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CourseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Course, error) {
	...
	if query := cq.withMedia; query != nil {
		if err := cq.loadMedia(ctx, query, nodes, nil,
			func(n *Course, e *Media) { n.Edges.Media = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCreator; query != nil {
		if err := cq.loadCreator(ctx, query, nodes, nil,
			func(n *Course, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCourseSections; query != nil {
		if err := cq.loadCourseSections(ctx, query, nodes,
			func(n *Course) { n.Edges.CourseSections = []*CourseSection{} },
			func(n *Course, e *CourseSection) { n.Edges.CourseSections = append(n.Edges.CourseSections, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCourseVideos; query != nil {
	...
	if query := cq.withTests; query != nil {
		if err := cq.loadTests(ctx, query, nodes,
			func(n *Course) { n.Edges.Tests = []*Test{} },
			func(n *Course, e *Test) { n.Edges.Tests = append(n.Edges.Tests, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CourseQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*Course, init func(*Course), assign func(*Course, *Media)) error {
...
func (cq *CourseQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Course, init func(*Course), assign func(*Course, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Course)
	for i := range nodes {
...
func (cq *CourseQuery) loadCourseSections(ctx context.Context, query *CourseSectionQuery, nodes []*Course, init func(*Course), assign func(*Course, *CourseSection)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(coursesection.FieldCourseID)
	}
	query.Where(predicate.CourseSection(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(course.CourseSectionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CourseQuery) loadCourseVideos(ctx context.Context, query *VideoQuery, nodes []*Course, init func(*Course), assign func(*Course, *Video)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Course)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(video.FieldCourseID)
	}
...
func (cq *CourseQuery) loadTests(ctx context.Context, query *TestQuery, nodes []*Course, init func(*Course), assign func(*Course, *Test)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Course)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(test.FieldCourseID)
	}
	query.Where(predicate.Test(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(course.TestsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CourseQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withMedia != nil {
			_spec.Node.AddColumnOnce(course.FieldMediaID)
		}
		if cq.withCreator != nil {
			_spec.Node.AddColumnOnce(course.FieldCreatorID)
		}
	}

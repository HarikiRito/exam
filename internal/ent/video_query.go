...
func (vq *VideoQuery) QueryCourseSection() *CourseSectionQuery {
	query := (&CourseSectionClient{config: vq.config}).Query()
...
func (vq *VideoQuery) QueryMedia() *MediaQuery {
	query := (&MediaClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryVideoQuestionTimestampsVideo chains the current query on the "video_question_timestamps_video" edge.
func (vq *VideoQuery) QueryVideoQuestionTimestampsVideo() *VideoQuestionTimestampQuery {
	query := (&VideoQuestionTimestampClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VideoQuery) WithCourseSection(opts ...func(*CourseSectionQuery)) *VideoQuery {
	query := (&CourseSectionClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withCourseSection = query
	return vq
}
...
func (vq *VideoQuery) WithMedia(opts ...func(*MediaQuery)) *VideoQuery {
	query := (&MediaClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithVideoQuestionTimestampsVideo tells the query-builder to eager-load the nodes that are connected to
// the "video_question_timestamps_video" edge. The optional arguments are used to configure the query builder of the edge.
func (vq *VideoQuery) WithVideoQuestionTimestampsVideo(opts ...func(*VideoQuestionTimestampQuery)) *VideoQuery {
	query := (&VideoQuestionTimestampClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withVideoQuestionTimestampsVideo = query
	return vq
}
...
func (vq *VideoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Video, error) {
	...
	if query := vq.withCourseSection; query != nil {
		if err := vq.loadCourseSection(ctx, query, nodes, nil,
			func(n *Video, e *CourseSection) { n.Edges.CourseSection = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := vq.withVideoQuestionTimestampsVideo; query != nil {
		if err := vq.loadVideoQuestionTimestampsVideo(ctx, query, nodes,
			func(n *Video) { n.Edges.VideoQuestionTimestampsVideo = []*VideoQuestionTimestamp{} },
			func(n *Video, e *VideoQuestionTimestamp) {
				n.Edges.VideoQuestionTimestampsVideo = append(n.Edges.VideoQuestionTimestampsVideo, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (vq *VideoQuery) loadVideoQuestionTimestampsVideo(ctx context.Context, query *VideoQuestionTimestampQuery, nodes []*Video, init func(*Video), assign func(*Video, *VideoQuestionTimestamp)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(videoquestiontimestamp.FieldVideoID)
	}
	query.Where(predicate.VideoQuestionTimestamp(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(video.VideoQuestionTimestampsVideoColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (vq *VideoQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := vq.ctx.Fields; len(fields) > 0 {
		...
		if vq.withCourseSection != nil {
			_spec.Node.AddColumnOnce(video.FieldSectionID)
		}
		if vq.withMedia != nil {

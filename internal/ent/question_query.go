...
// QueryQuestionOptions chains the current query on the "question_options" edge.
func (qq *QuestionQuery) QueryQuestionOptions() *QuestionOptionQuery {
...
// QueryVideoQuestionTimestampsQuestion chains the current query on the "video_question_timestamps_question" edge.
func (qq *QuestionQuery) QueryVideoQuestionTimestampsQuestion() *VideoQuestionTimestampQuery {
	query := (&VideoQuestionTimestampClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QuestionQuery) QueryTestIgnoreQuestions() *TestIgnoreQuestionQuery {
	query := (&TestIgnoreQuestionClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithQuestionOptions tells the query-builder to eager-load the nodes that are connected to
// the "question_options" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionQuery) WithQuestionOptions(opts ...func(*QuestionOptionQuery)) *QuestionQuery {
	query := (&QuestionOptionClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withQuestionOptions = query
	return qq
}

// WithVideoQuestionTimestampsQuestion tells the query-builder to eager-load the nodes that are connected to
// the "video_question_timestamps_question" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionQuery) WithVideoQuestionTimestampsQuestion(opts ...func(*VideoQuestionTimestampQuery)) *QuestionQuery {
	query := (&VideoQuestionTimestampClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (qq *QuestionQuery) WithTestIgnoreQuestions(opts ...func(*TestIgnoreQuestionQuery)) *QuestionQuery {
	query := (&TestIgnoreQuestionClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (qq *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	...
	if query := qq.withQuestionOptions; query != nil {
		if err := qq.loadQuestionOptions(ctx, query, nodes,
			func(n *Question) { n.Edges.QuestionOptions = []*QuestionOption{} },
			func(n *Question, e *QuestionOption) { n.Edges.QuestionOptions = append(n.Edges.QuestionOptions, e) }); err != nil {
			return nil, err
		}
	}
	if query := qq.withVideoQuestionTimestampsQuestion; query != nil {
		if err := qq.loadVideoQuestionTimestampsQuestion(ctx, query, nodes,
	...
	if query := qq.withTestIgnoreQuestions; query != nil {
		if err := qq.loadTestIgnoreQuestions(ctx, query, nodes,
...
func (qq *QuestionQuery) loadQuestionOptions(ctx context.Context, query *QuestionOptionQuery, nodes []*Question, init func(*Question), assign func(*Question, *QuestionOption)) error {
	...
	query.Where(predicate.QuestionOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(question.QuestionOptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (qq *QuestionQuery) loadVideoQuestionTimestampsQuestion(ctx context.Context, query *VideoQuestionTimestampQuery, nodes []*Question, init func(*Question), assign func(*Question, *VideoQuestionTimestamp)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Question)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(videoquestiontimestamp.FieldQuestionID)
	}
...
func (qq *QuestionQuery) loadTestIgnoreQuestions(ctx context.Context, query *TestIgnoreQuestionQuery, nodes []*Question, init func(*Question), assign func(*Question, *TestIgnoreQuestion)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Question)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(testignorequestion.FieldQuestionID)
	}
